...
def juntar_pdfs(pasta_entrada, arquivo_saida, pasta_individual, log, progress, btn_iniciar):
    arquivos = [
    ...
    pdfs_temp = []  # lista de (nome_original, caminho_temp)
    ...
    for i, nome in enumerate(arquivos):
        ...
        try:
            ...
            # Salva com o mesmo nome original na pasta temporária
            temp_nome = os.path.join(pasta_temp, nome)
            with open(temp_nome, "wb") as f:
                writer.write(f)
 
            pdfs_temp.append((nome, temp_nome))
            log(f"✔  {nome}")
    ...
    # Ordena pelo primeiro número do nome antes de unir no arquivo final
    pdfs_temp_ordenados = sorted(pdfs_temp, key=lambda x: ordenar_numerico(x[0]))
 
    writer_final = PdfWriter()
    for _, caminho_temp in pdfs_temp_ordenados:
    ...
    # Copia arquivos individuais (primeira página) para pasta de saída individual
    if pasta_individual:
        os.makedirs(pasta_individual, exist_ok=True)
        for nome, caminho_temp in pdfs_temp:
            destino = os.path.join(pasta_individual, nome)
            shutil.copy2(caminho_temp, destino)
        log(f"📁 Individuais salvos em: {pasta_individual}")
 
    shutil.rmtree(pasta_temp, ignore_errors=True)
 
    progress["value"] = 100
    log(f"\n✅ Concluído! {len(pdfs_temp_ordenados)} página(s) unidas.")
    log(f"📄 Salvo em: {arquivo_saida}")
    messagebox.showinfo("Sucesso", f"Arquivo gerado com sucesso!\n\n{arquivo_saida}")
    btn_iniciar.config(state="normal")
 
 
def iniciar_thread(entrada_var, saida_var, individual_var, log, progress, btn_iniciar):
    pasta = entrada_var.get().strip()
    saida = saida_var.get().strip()
    pasta_individual = individual_var.get().strip() or None
    ...
    threading.Thread(
        target=juntar_pdfs,
        args=(pasta, saida, pasta_individual, log, progress, btn_iniciar),
        daemon=True
    ).start()
...
# Arquivo de saída
tk.Label(root, text="Salvar resultado como (arquivo unificado):", font=("Segoe UI", 9)).grid(
...
# Pasta para arquivos individuais (opcional)
tk.Label(root, text="Salvar páginas individuais em (opcional):", font=("Segoe UI", 9)).grid(
    row=5, column=0, columnspan=3, sticky="w"
)
individual_var = tk.StringVar()
tk.Entry(root, textvariable=individual_var, width=55, font=("Segoe UI", 9)).grid(
    row=6, column=0, columnspan=2, sticky="ew", pady=(0, PADY)
)
tk.Button(
    root, text="Selecionar", font=("Segoe UI", 9),
    command=lambda: individual_var.set(filedialog.askdirectory(title="Pasta para páginas individuais"))
).grid(row=6, column=2, padx=(6, 0), pady=(0, PADY))
...
log_widget.grid(row=8, column=0, columnspan=3, pady=(0, PADY))
...
progress.grid(row=9, column=0, columnspan=3, pady=(0, PADY), sticky="ew")
 
# Botão iniciar
btn_iniciar = tk.Button(
    root, text="▶  Iniciar", font=("Segoe UI", 10, "bold"),
    bg="#0078D4", fg="white", relief="flat", padx=16, pady=6,
    command=lambda: iniciar_thread(entrada_var, saida_var, individual_var, log, progress, btn_iniciar)
)
btn_iniciar.grid(row=10, column=0, columnspan=3, pady=(4, 0))
