...
def juntar_pdfs(pasta_entrada, log, progress, btn_iniciar):
    arquivo_saida = os.path.join(pasta_entrada, "Arquivo_Final.pdf")
...
def iniciar_thread(entrada_var, log, progress, btn_iniciar):
    ...
    threading.Thread(
        target=juntar_pdfs,
        args=(pasta, log, progress, btn_iniciar),
        daemon=True
    ).start()
...
log_widget.grid(row=4, column=0, columnspan=3, pady=(0, PADY))
...
progress.grid(row=5, column=0, columnspan=3, pady=(0, PADY), sticky="ew")
 
# Botão iniciar
btn_iniciar = tk.Button(
    root, text="▶  Iniciar", font=("Segoe UI", 10, "bold"),
    bg="#0078D4", fg="white", relief="flat", padx=16, pady=6,
    command=lambda: iniciar_thread(entrada_var, log, progress, btn_iniciar)
)
btn_iniciar.grid(row=6, column=0, columnspan=3, pady=(4, 0))
